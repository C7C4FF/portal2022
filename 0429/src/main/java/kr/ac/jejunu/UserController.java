...
public class UserController {
    @RequestMapping(path = "/user/{id}/{name}", produces = "application/json")
    ...
    public Cookie[] cookie(@CookieValue("khb") HttpServletRequest request, HttpServletResponse response){
        Cookie cookie = new Cookie("khb", "1234");
    ...
    public User session(HttpSession httpSession){
        ...
        if (user == null)
            httpSession.setAttribute("user", User.builder().name("session").password("1111").build());
